...
static func create_new(code: String, type: Type, amount: float, currency: Currency, party: Party) -> Charge:
	var new_charge: Charge = Charge.new()
	
	new_charge.charge_data = ChargeData.get_charge_by_code(code)
	new_charge.type = type
	new_charge.amount = amount
	new_charge.currency = currency
	new_charge.party = party
	
	return new_charge


@warning_ignore("shadowed_variable")
static func create_new_from_cost_with_margin(charge_cost: Charge, margin_percentage: float, margin_amount: float, party: Party) -> Charge:
	return create_new(charge_cost.charge_data.code, Type.REVENUE, charge_cost.amount * (1 + margin_percentage) + margin_amount, charge_cost.currency, party)
