...
static func create_new() -> Person:
	var gender: Gender = GlobalRefs.genders.pick_random()
	var first_name: String
	if gender.gender_name == "male":
		first_name = MALE_NAMES.pick_random()
	elif gender.gender_name == "female":
		first_name = FEMALE_NAMES.pick_random()
	var last_name: String = SURNAMES.pick_random()
	
	var new_person: Person = Person.new().with_data(
		GlobalRefs.get_person_id(),
		first_name,
		last_name,
		gender,
		"%s.%s@%s" % [first_name.to_lower(), last_name.to_lower(), "email.com"],
		"%s" % [randi_range(500000000, 999999999)],
		"%04d-%02d-%02d" % [randi_range(1950, 2005), randi_range(1, 12), randi_range(1, 28)], ##TODO: fix so that it can be up to 31 day of the month
		randi_range(0, Person.Experience.size() - 1),
	...
	if new_person.employer != null:
		new_person.email = "%s.%s@%s" % [first_name.to_lower(), last_name.to_lower(), new_person.employer.domain]
