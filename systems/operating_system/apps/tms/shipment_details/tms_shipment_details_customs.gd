...
func _on_arrange_export_customs_button_pressed() -> void:
	...
	shipment.events.register_new_planned_event("CSE", GlobalTimer.get_future_date_from_event(shipment.events.get_first_event_of_code("RCV"), 1, 8))
	refresh()
...
func _on_arrange_import_customs_button_pressed() -> void:
	...
	shipment.events.register_new_planned_event("CSI", GlobalTimer.get_future_date_from_event(shipment.events.get_first_event_of_code("ARR"), 0, 20))
	refresh()
