...
func _on_arrange_main_freight_button_pressed() -> void:
	...
	shipment.events.create_new_planned_event("DEP", GlobalTimer.get_future_date_from_event(shipment.events.get_first_event_of_code("ERL"), 1, 12))
	shipment.events.create_new_planned_event("ARR", GlobalTimer.get_future_date_from_event(shipment.events.get_first_event_of_code("LTS"), -1, 12))
	refresh()
