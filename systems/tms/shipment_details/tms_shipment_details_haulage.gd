...
func _on_arrange_pickup_button_pressed() -> void:
	...
	shipment.events.create_new_planned_event("PUP", GlobalTimer.get_future_date_from_event(shipment.events.get_first_event_of_code("ERL"), 0, 12))
	shipment.events.create_new_planned_event("RCV", GlobalTimer.get_future_date_from_event(shipment.events.get_first_event_of_code("PUP"), 0, 17))
	refresh()


func _on_arrange_delivery_button_pressed() -> void:
	shipment.haulage.trucker_delivery = GlobalRefs.truckers.pick_random()
	shipment.events.create_new_planned_event("REL", GlobalTimer.get_future_date_from_event(shipment.events.get_first_event_of_code("LTS"), 0, 6))
	shipment.events.create_new_planned_event("DEL", GlobalTimer.get_future_date_from_event(shipment.events.get_first_event_of_code("REL"), 0, 8))
	refresh()
