...
func refresh_shipment_list_items() -> void:
	...
	var shipment_list: Array[Shipment]
	if show_completed:
		shipment_list = GameManager.player.employer.shipments
	else:
		shipment_list = GameManager.player.employer.shipments.filter(Shipment.is_shipment_not_completed)
