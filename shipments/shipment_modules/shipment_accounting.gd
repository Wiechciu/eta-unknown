...
func create_new_revenue_charge(code: Charge.Code, amount: float, currency: Currency, party: Party) -> Charge:
	var new_charge: Charge = Charge.new().with_data(code, Charge.Type.REVENUE, amount, currency, party)
	register_charge(new_charge)
	return new_charge


func create_new_cost_charge(code: Charge.Code, amount: float, currency: Currency, party: Party) -> Charge:
	var new_charge: Charge = Charge.new().with_data(code, Charge.Type.COST, amount, currency, party)
	register_charge(new_charge)
	return new_charge


func get_all_charges_of_code(code: Charge.Code) -> Array[Charge]:
	return charges.filter(func(charge: Charge) -> bool: return charge.code == code)


func get_all_revenue_charges_of_code(code: Charge.Code) -> Array[Charge]:
	return revenue_charges.filter(func(charge: Charge) -> bool: return charge.code == code)


func get_all_cost_charges_of_code(code: Charge.Code) -> Array[Charge]:
	return cost_charges.filter(func(charge: Charge) -> bool: return charge.code == code)
